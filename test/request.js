    ...
    before(function(done) {
        ...
        db.sequelize.sync({
            ...
            async.series([
                ...
                function(callback) {
                    dbUtils.createCASUser(db, users[0].username, callback);
                },
                function(callback) {
                    dbUtils.createCASUser(db, users[1].username, callback);
                }
            ], function() {
                done();
            });
        ...
        describe('create requests', function() {
            it('should create a request in the queue', function(done) {
                ...
                req.end(function(error,
                    ...
                    assert.equal(res.status, requestStatuses.in_queue);
                    done();
                });
            });
            ...
            it('should not allow a non-TA to claim the request', function(done) {
                var req = requestUtils[1].createRequest(server,
                    '/api/v1/queue/test-queue/requests',
                    'POST', {});
